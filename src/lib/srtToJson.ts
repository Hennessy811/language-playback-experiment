 ...
 */
export default function parseSRT(
  data = "",
  options = { propName: {} }
): {
  id: number
  ...
  for (let i = 0; i < endIdx; i++) {
    ...
    sub[options.propName.start || "start"] = options.timeInText
      ? time[0]
      : toSeconds(time[0])
    ...
    sub[options.propName.end || "end"] = options.timeInText
      ? time[1]
      : toSeconds(time[1])
    ...
    sub[textPropName] = sub[textPropName]
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
    ...
    sub[textPropName] = sub[textPropName].replace(
